...
///
/// \brief The ImageUndistort class is the main class that perform image undistortion
///
...
public:
    ...
    /// \param nh_private_ private node handle
    ///
...
private:
    ...
    ///
    /// \brief Callback function for input image
    /// \param image_msg_in the input image as a ROS image
    ///
    ...
    image_transport::CameraSubscriber camera_sub_;  ///< input image subscriber (image and cam_info)
    ros::Subscriber camera_info_sub_;               ///< output camera_info

    // publishers
    image_transport::CameraPublisher camera_pub_;  ///< output camera publisher (image and cam_info)
    ...
    /// object that holds camera info of the input and virtual output camera
    std::shared_ptr<CameraParametersPair> camera_parameters_pair_ptr_;

    ///
    /// \brief Enumeration for the source of output camera info
    ///
    enum class OutputInfoSource {
        AUTO_GENERATED, ///< automatically generated based on the input, the
        ///< focal length is the average of fx and fy of the input, the center point is
        ///< in the center of the image, R=I and only x-translation is preserved.
        ///< Resolution is set to the largest area that contains no empty pixels. The
        ///< size of the output can also be modified with the "scale" parameter
        MATCH_INPUT,    ///< intrinsic values are taken from the input camera parameters
        ROS_PARAMS,     ///< values are loaded from ros parameters, usually stored in an xml file.
        ///< The file format matches that generated by Kalibr.
        CAMERA_INFO     ///< the output parameters are found through subscribing to a camera_info ros topic
    };
